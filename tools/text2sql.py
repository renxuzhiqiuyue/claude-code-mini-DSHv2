"""text2sql：MySQL 只读查询相关工具（同域合并，风格同 filesystem）。"""
...
def _fetch_rows(query: str, max_rows: int = 100_000) -> tuple[list[str], list[dict]]:
    return get_mysql_manager().fetch_query_rows(query, max_rows=max_rows)


@tool("sql_db_list_tables")
def sql_db_list_tables() -> str:
    """列出 MySQL 数据库中的所有表名及其描述信息。"""
    print("\033[33m→ sql_db_list_tables()\033[0m")
    try:
        tables_info = get_mysql_manager().get_tables_with_comments()
...
def sql_db_query(query: str) -> str:
    ...
    try:
        out = get_mysql_manager().execute_query(query)
    except Exception as e:
...
def sql_db_query_checker(query: str) -> str:
    ...
    try:
        out = get_mysql_manager().validate_query(query)
    except Exception as e:
